...
AST::AST(const AST& actual) {
    ...
    //into this.
    tag = actual.tag; //copies tag
    nodeType = actual.nodeType; //copies nodetype
    text = actual.text; //copies text
    closeTag = actual.closeTag; //copies closetag

    if(actual.child.empty()){
        return;
    }
    for(std::list<AST*>::const_iterator i = actual.child.begin(); i != actual.child.end(); ++i) {
        child.push_back(new AST(**i)); //pushback pointer
    }
}
...
void AST::swap(AST& rhs) {
    //TODO: IMPLEMENT 
    AST temp(rhs); rhs.nodeType = nodeType; rhs.tag = tag;
    rhs.closeTag = closeTag; rhs.text = text; nodeType = temp.nodeType;
    tag = temp.tag; closeTag = temp.closeTag; text = temp.text;
    rhs.child = child; child = temp.child;
...
void AST::mainHeader(const std::vector<std::string>& profileName,
    ...
    std::list<AST*>::iterator ptr = child.begin();
    while((*ptr)->tag != "function" && ptr != child.end()) {++ptr; } 
...
void AST::mainReport(const std::vector<std::string>& profileName) {
    ...
    while((*blockptr)->tag != "return" && blockptr != (*ptr)->getChild("block")->getChild("block_content")->child.begin()) {--blockptr; }
        if(blockptr == (*ptr)->getChild("block")->getChild("block_content")->child.begin()) blockptr = --(*ptr)->getChild("block")->getChild("block_content")->child.end();
             for(unsigned i = 0; i < profileName.size(); ++i) {
